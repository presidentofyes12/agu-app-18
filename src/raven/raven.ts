...
interface RavenConnectionStatus {
  connected: boolean;
  relays: string[];
  writeRelays: string[];
  readRelays: string[];
...
class Raven extends TypedEventEmitter<RavenEvents, EventHandlerMap> {
  ...
  public async createDiscussionThread(params: {
    ...
    await this.publish(Kind.ChannelMessage, [['e', params.proposalId]], content);
  }
  ...
  public async addDiscussionComment(params: {
    proposalId: string;
    content: string;
    timestamp: number;
  }): Promise<void> {
  ...
  public async updateProposalContent(params: {
    proposalId: string;
    title?: string;
    description?: string;
    category?: number;
    ...
    await this.publish(Kind.ChannelMetadata, [['e', params.proposalId]], content);
  }

  public async getDiscussionEvents(proposalId: string): Promise<DiscussionEvent[]> {
...
public async sendDirectMessage(
  ...
  public async checkConnectionStatus(): Promise<RavenConnectionStatus> {
    const writeRelays = this.writeRelays || [];
    const readRelays = this.readRelays || [];
